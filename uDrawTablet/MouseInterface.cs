...
namespace uDrawTablet
{
	public static class MouseInterface
	{
		...
		{
			foreach (var t in Tablets)
				t.Settings = TabletSettings.LoadSettings (GetSettingsFileName (t.Tablet as WiiInputDevice != null, t.Tablet as PS3InputDevice != null, t.ReceiverIndex));
		}
		...
		private static void _PerformAction (TabletConnection conn, TabletOptionButton.TabletButton button, TabletOptionButton.ButtonAction action, bool held)
		{
		...
		private static void _Handle360TabletConnect (int index)
		{
			...
			var connection = new TabletConnection ((new Xbox360InputDevice (Receiver, index, Receiver.GetDeviceInformation (index))) as IInputDevice, Receiver, index);
		...
		private static void _HandleTabletEvents (TabletConnection conn)
		{
			if (conn != null) {
				...
				double threshold = ((conn.Settings.PenPressureThreshold / 10.0) * (_MAX_PEN_PRESSURE_THRESHOLD - _MIN_PEN_PRESSURE_THRESHOLD)) + _MIN_PEN_PRESSURE_THRESHOLD;
				...
				if (conn.Settings.PPJoyNumber > 0) {
					...
					if (conn.Tablet.PressureType == TabletPressureType.FingerPressed || conn.Tablet.PressureType == TabletPressureType.PenPressed) {
				...
				if ((conn.Tablet.PressureType == TabletPressureType.PenPressed) || (conn.Settings.AllowFingerMovement && conn.Tablet.PressureType == TabletPressureType.FingerPressed)) {
					if (conn.Settings.MovementType == TabletSettings.TabletMovementType.Absolute) {
						...
						if (conn.Settings.RestrictToCurrentWindow) {
							...
							var v = SystemInformation.VirtualScreen;
							width = (float)Math.Round ((float)((float)conn.CurrentDisplay.Bounds.Width / (float)v.Width) * 65536.0, 0);
							height = (float)Math.Round ((float)((float)conn.CurrentDisplay.Bounds.Height / (float)v.Height) * 65536.0, 0);
							xStart = ((((float)conn.CurrentDisplay.Bounds.X - (float)v.X) / (float)v.Width) * (float)65536.0);
							yStart = ((((float)conn.CurrentDisplay.Bounds.Y - (float)v.Y) / (float)v.Height) * (float)65536.0);
						}
						
						if (conn.Settings.MaintainAspectRatio) {
							//Get the current style based on which is higher, width or height
							var style = ((tabletPadHeight / tabletPadWidth) >= (actualHeight / actualWidth)) ? DockOption.DockStyle.Vertical : DockOption.DockStyle.Horizontal;
							
							//Translate the width and height to tablet proportions
							float tabletWidth = (style == DockOption.DockStyle.Vertical ? tabletPadWidth : tabletPadWidth * ((tabletPadHeight / tabletPadWidth) / (actualHeight / actualWidth)));
							float tabletHeight = (style == DockOption.DockStyle.Horizontal ? tabletPadHeight : tabletPadHeight * ((actualHeight / actualWidth) / (tabletPadHeight / tabletPadWidth)));
							float tabletX = 0;
							float tabletY = 0;
							...
							if (x < tabletX)
								x = tabletX;
							if (x > (tabletX + tabletWidth))
								x = tabletX + tabletWidth;
							if (y < tabletY)
								y = tabletY;
							if (y > (tabletY + tabletHeight))
								y = tabletY + tabletHeight;
						...
						if (Cursor.Position.X != x && Cursor.Position.Y != y)
							//not sure if this respects virtual desktop coordinates
							mouse_event (MOUSEEVENTF_MOVE | MOUSEEVENTF_ABSOLUTE | MOUSEEVENTF_VIRTUALDESK, (int)x, (int)y, 0, UIntPtr.Zero);
		...
		private static void _CheckKeys (TabletOptionButton.TabletButton button, byte p, bool ctrl, bool shift, bool alt, bool win, bool held, bool sendOnce)
		{
			try {
				if (held) {
					...
					if (_keyCounters[button].ContainsKey (p) && _keyCounters[button][p] > 0) {
						_keyCounters[button][p]--;
